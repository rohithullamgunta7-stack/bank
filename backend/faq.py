...
# # Add this endpoint to your faq.py file (around line 120, after get_all_faqs)

# @router.get("/", response_model=List[FAQResponse])
# async def get_faqs_simple(
#     current_user: dict = Depends(get_current_user)
# ):
#     """Get all active FAQs - simple endpoint for FAQ management UI"""
#     if not mongo_connected or faq_collection is None:
#         raise HTTPException(status_code=503, detail="Database unavailable")
    
#     try:
#         # Get all FAQs, sorted by creation date (newest first)
#         faqs = list(faq_collection.find({}).sort("created_at", -1))
        
#         return [
#             FAQResponse(
#                 faq_id=faq.get("faq_id", str(faq["_id"])),
#                 question=faq["question"],
#                 answer=faq["answer"],
...
#                 not_helpful_count=faq.get("not_helpful_count", 0),
#                 is_active=faq.get("is_active", True),
#                 created_at=faq.get("created_at", datetime.now(timezone.utc).isoformat()),
#                 updated_at=faq.get("updated_at")
#             )
#             for faq in faqs
#         ]
...
# # Also update your create endpoint to ensure it returns proper data
# @router.post("/", response_model=FAQResponse)
# async def create_faq_simple(
#     faq: FAQCreate,
#     current_user: dict = Depends(require_support_or_admin)
# ):
#     """Create a new FAQ entry - simple endpoint"""
#     if not mongo_connected or faq_collection is None:
#         raise HTTPException(status_code=503, detail="Database unavailable")
...
#             "category": faq.category,
#             "tags": faq.tags,
#             "source": faq.source if hasattr(faq, 'source') else "manual",
#             "usage_count": 0,
#             "helpful_count": 0,
#             "not_helpful_count": 0,
#             "is_active": True,
...
# # Update your update endpoint
# @router.put("/", response_model=FAQResponse)
# async def update_faq_simple(
#     faq_id: str,
#     question: str,
#     answer: str,
#     current_user: dict = Depends(require_support_or_admin)
# ):
#     """Update an existing FAQ - simple endpoint"""
#     if not mongo_connected or faq_collection is None:
#         raise HTTPException(status_code=503, detail="Database unavailable")
...
# # Delete endpoint
# @router.delete("/")
# async def delete_faq_simple(
#     faq_id: str,
#     current_user: dict = Depends(require_admin)
# ):
#     """Delete an FAQ - simple endpoint"""
#     if not mongo_connected or faq_collection is None:
#         raise HTTPException(status_code=503, detail="Database unavailable")
    
#     try:
#         result = faq_collection.delete_one({"faq_id": faq_id})
