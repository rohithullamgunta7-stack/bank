...
def get_current_user(authorization: Optional[str] = Header(None)):
    ...
    print(f"✅ Authenticated user: {user_data['email']} (role: {user_data.get('role')})")
    return user_data
...
def signup(user: UserSignup):
    ...
    try:
        ...
        user_doc = {
            ...
            "password": hashed_pw, 
            "full_name": name,
            "name": name,  # Store in both fields for compatibility
            "role": "user",
            "created_at": datetime.utcnow().isoformat(),
            "last_login": datetime.utcnow().isoformat()
        }
        users_collection.insert_one(user_doc)
        print(f"✅ User created: {email} (role: user)")
        
        token = create_access_token({"sub": email, "name": name, "role": "user"})
        return {"access_token": token, "token_type": "bearer"}
...
def admin_signup(user: AdminSignup):
    ...
    print(f"🔐 Admin signup attempt: {user.email} (role: {user.role})")
    
    if user.admin_secret != ADMIN_SECRET_KEY:
        print(f"❌ Invalid admin secret key provided")
        raise HTTPException(status_code=403, detail="Invalid admin secret key")
    ...
    try:
        ...
        print(f"✅ Admin/Support user created: {email} (role: {user.role}, id: {user_id})")
        
        token = create_access_token({"sub": email, "name": name, "role": user.role})
        return {"access_token": token, "token_type": "bearer"}
    
    except Exception as e:
        print(f"❌ Admin signup error: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail="Signup failed. Please try again.")
...
def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    print(f"✅ User found: {email} (role: {db_user.get('role')})")
    ...
    # Get user name - check multiple fields
    user_name = db_user.get("full_name") or db_user.get("name", "User")
    user_role = db_user.get("role", "user")
    
    print(f"✅ Login successful: {email} (name: {user_name}, role: {user_role})")
...
def get_current_user_info(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # Handle created_at
        created_at_value = db_user.get("created_at")
...
def update_user_role(request: RoleUpdateRequest, admin_user: dict = Depends(require_admin)):
    ...
    return {
        "message": f"User {user_name} role updated to {request.new_role}",
        "user_id": request.user_id,
        "new_role": request.new_role
    }
