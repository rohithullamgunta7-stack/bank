...
def connect_mongodb(retry_count=0):
    ...
    global orders_col, refunds_col
    # --- ADDED COLLECTIONS ---
    global session_cache_collection, user_context_collection
    # -------------------------
    ...
    try:
        ...
        mongo_client = MongoClient(
            MONGO_URI,
            tls=True,
            tlsAllowInvalidCertificates=True, # Note: Be cautious with this in strict production
            serverSelectionTimeoutMS=10000,
            connectTimeoutMS=20000,
            socketTimeoutMS=20000,
            retryWrites=True,
            retryReads=True,
            maxPoolSize=100, # Increased pool size for more workers
            minPoolSize=5
        )
        ...
        # Get database
        db = mongo_client.get_database() # Get DB from URI
        print(f"📦 Connected to database: {db.name}")
        ...
        refunds_col = db["refunds"]
        # --- ADDED COLLECTIONS ---
        session_cache_collection = db["session_cache"]
        user_context_collection = db["user_contexts"]
        # -------------------------
...
def create_indexes():
    ...
    try:
        ...
        # --- ADDED TTL INDEXES for worker-proof cache ---
        
        # User Context TTL Index (auto-expires documents)
        user_context_collection.create_index(
            "expires_at",
            expireAfterSeconds=0
        )
        
        # Session Cache TTL Index (auto-expires documents)
        session_cache_collection.create_index(
            "expires_at",
            expireAfterSeconds=0
        )
        # ------------------------------------------------
        ...
        # Accounts indexes
        accounts_col.create_index([("customer_id", 1)])
        accounts_col.create_index("account_number", unique=True, sparse=True)
        accounts_col.create_index("account_type")
        accounts_col.create_index("status")

        # Transactions indexes
        transactions_col.create_index([("account_id", 1), ("date", -1)])
        transactions_col.create_index("transaction_id", unique=True, sparse=True)
        transactions_col.create_index("status")
        transactions_col.create_index("type")
        ...
        # Feedback indexes
        feedback_collection.create_index([("user_id", 1), ("submitted_at", -1)])
        feedback_collection.create_index("feedback_id", unique=True, sparse=True)
        feedback_collection.create_index("rating")
        feedback_collection.create_index("submitted_at")

        # Chat sessions indexes
        chat_sessions_collection.create_index([("user_id", 1), ("start_time", -1)])
        chat_sessions_collection.create_index("session_id", unique=True, sparse=True)
        chat_sessions_collection.create_index("feedback_submitted")
        ...
        # Conversation patterns indexes
        conversation_patterns_collection.create_index("pattern_id", unique=True, sparse=True)
        conversation_patterns_collection.create_index("approved")

        # Escalations indexes
        escalations_collection.create_index("escalation_id", unique=True, sparse=True)
        escalations_collection.create_index([("user_id", 1), ("created_at", -1)])
        escalations_collection.create_index("status")
        escalations_collection.create_index("assigned_to")
        ...
        print(f"⚠️ Warning: Index creation failed: {str(e)}")
        pass # Continue running even if index creation fails
