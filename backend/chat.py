...
# ===================================================================
# MONGO-DB BASED CONTEXT (Worker-Proof)
# ===================================================================
user_context_collection = db["user_contexts"] if db is not None else None
CONTEXT_DURATION = timedelta(minutes=10)

def set_user_context(user_id, context_type, context_data):
    """Store user's current context in MongoDB (shared across workers)"""
    if user_context_collection is None:
        print("⚠️ MongoDB unavailable - context not saved!")
        return
        ...
        user_context_collection.update_one(
            {"user_id": user_id},
            {
                "$set": {
                    ...
                    "data": context_data,
                    "timestamp": datetime.now(timezone.utc),
                    "expires_at": datetime.now(timezone.utc) + CONTEXT_DURATION
                }
            },
            upsert=True
        )
        print(f"✅ MongoDB: Context set for {user_id}: {context_type}")
    except Exception as e:
        print(f"❌ Error setting context: {e}")


def get_user_context(user_id):
    """Retrieve user's current context from MongoDB if not expired"""
    if user_context_collection is None:
        return None
    
    try:
        context = user_context_collection.find_one({
            "user_id": user_id,
            "expires_at": {"$gte": datetime.now(timezone.utc)}
        })
        
        if context:
            print(f"✅ MongoDB: Context retrieved for {user_id}: {context['type']}")
            return {
                "type": context["type"],
                "data": context["data"],
        ...
        # Note: We no longer need to manually delete.
        # The TTL index in database.py handles automatic expiration.
        print(f"⏰ Context expired or not found for {user_id}")
        return None
...
def clear_user_context(user_id):
    ...
    try:
        user_context_collection.delete_one({"user_id": user_id})
        print(f"🗑️ MongoDB: Context cleared for {user_id}")
    except Exception as e:
...
transactions_col = db["transactions"] if db is not None else None
customers_col = db["customers"] if db is not None else None
session_cache_collection = db["session_cache"] if db is not None else None

CACHE_DURATION = timedelta(minutes=15)

# ===================================================================
# MONGO-DB BASED CACHE (Worker-Proof)
# ===================================================================

def get_cached_history(user_id, limit=30):
    """Retrieve recent conversation history from MongoDB cache or database."""
    if session_cache_collection is None:
        # Fallback to direct DB load if cache is unavailable
        return load_chat_history(user_id, limit=limit)

    now = datetime.now(timezone.utc)
    
    try:
        cached = session_cache_collection.find_one({
            "user_id": user_id,
            "expires_at": {"$gte": now}
        })
    ...
    # Save the new history to the cache
    try:
        session_cache_collection.update_one(
            {"user_id": user_id},
            {
                "$set": {
                    "history": history,
...
def invalidate_cache(user_id):
    """Clear cache from MongoDB when new message is saved."""
    if session_cache_collection is None:
        return
    try:
        session_cache_collection.delete_one({"user_id": user_id})
        print(f"✅ Cache invalidated for user: {user_id}")
    except Exception as e:
        print(f"❌ Error invalidating cache: {e}")

# NOTE: cleanup_cache() is no longer needed.
# The TTL index we will add to database.py handles this automatically.
            ...
            if user_msg.strip() in ["1", "temporary"]:
                ...
                escalation_id = create_escalation(
                    ...
                    f"URGENT: User requested PERMANENT card cancellation - {card_type} - Account {account_number}",
                    {
                        ...
                        "phone_number": user_details.get('phone_number', 'Not Available'),
                        "address": user_details.get('address', 'Not Available'),
                        "customer_id": str(user_details.get('customer_id', user_id)),
                        "account_created_at": account_created_at,
                        "kyc_status": user_details.get('kyc_status', 'Unknown'),
                        "customer_tier": user_details.get('customer_tier', 'Unknown'),
                        "date_of_birth": user_details.get('date_of_birth', 'Not Available')
        ...
        try:
            ...
            if accounts_data is None:
                ...
                return {
                    "reply": "ACCOUNT_LIST",
                    "accounts": accounts_data,
                    "message": "Which account would you like to see details for?"
                }
        except Exception as e:
            print(f"Account balance error: {e}")
            traceback.print_exc()
            ...
            if accounts_data:
                ...
                if card_accounts:
                    ...
                    return {
                        ...
                        "accounts": card_accounts,
                        "action": "card_block",
                        "message": "Which card (linked to the account) would you like to block?"
                    }
                else:
                    bot_msg = (
                        "You don't have any active cards.\n\n"
