...
from .config import (
    ...
    session_cache_collection # Added for cache TTL
)
...
def init_db():
    ...
    try:
        # Create TTL index for automatic context cleanup
        if "user_contexts" in db.list_collection_names():
            user_contexts = db["user_contexts"]
            user_contexts.create_index(
                "expires_at",
                expireAfterSeconds=0  # MongoDB will auto-delete when expires_at is reached
            )
            print("✅ User context TTL index created")
            
        # Create TTL index for automatic session cache cleanup
        if "session_cache" in db.list_collection_names():
            session_cache = db["session_cache"]
            session_cache.create_index(
                "expires_at",
                expireAfterSeconds=0 # MongoDB will auto-delete when expires_at is reached
            )
            print("✅ Session cache TTL index created")
            
        # Create index for chat history
        if messages_collection is not None:
            messages_collection.create_index([("user_id", 1), ("timestamp", -1)])
            print("✅ Chat history index created")
...
def get_user_by_id(user_id):
    ...
    if user:
        ...
        # ✅ FIX: Convert ALL datetime objects to strings for JSON serialization
        if "created_at" in user:
            if isinstance(user["created_at"], datetime):
                user["created_at"] = user["created_at"].isoformat()
            elif user["created_at"] is None:
        ...
        # Ensure _id is a string for JSON
...
def get_customer_by_user_id(user_id):
    """Get customer with verification details"""
    if not mongo_connected or users_collection is None or customers_col is None:
        return None
        
    try:
        user = get_user_by_id(user_id) # This already handles finding the user
        if not user:
            return None
        ...
        # If no customer profile, return basic user info
        return user
    except Exception as e:
        print(f"Error getting customer: {e}")
        return None
...
def save_message(user_id, user_msg, bot_msg, agent_id=None, escalation_id=None):
    ...
    if not mongo_connected or messages_collection is None:
        return False
    try:
        # Standardize message saving to handle bot-only or user-only messages
        ...
        if bot_msg is not None:
            # Handle dictionary replies (e.g., account lists)
            content_to_save = bot_msg
            if isinstance(bot_msg, dict):
                content_to_save = bot_msg.get("reply", "ACCOUNT_LIST_RESPONSE")

            messages_collection.insert_one({
                "user_id": user_id,
                "role": "bot", # Use 'bot' or 'assistant' consistently
        ...
        #
        # ⚠️ CRITICAL: invalidate_cache() must be called from bot_logic.py
        # AFTER this function returns True, not from here.
        #
...
def load_chat_history(user_id, limit=30):
    """Load chat history for user, formatted for AI model."""
    if not mongo_connected or messages_collection is None:
        return []
    try:
        ...
        docs = list(query)
        # Reverse the limited list to get chronological order
        docs.reverse() 
        
        history = []
        for doc in docs:
            role = doc.get("role", "user") # Default to user if role is missing
            content = doc.get("content", "")
            
            if role == "user":
                history.append({"role": "user", "content": content})
            elif role == "bot" or role == "assistant":
                history.append({"role": "model", "content": content}) # Use 'model' for AI
    ...
    try:
        result = accounts_col.update_one(
            {"_id": ObjectId(account_id)},
            {
                "$set": {
                    "card_status": "blocked_temporary",
                    "status": "Active (Card Blocked)", # Update main status too
                    "card_blocked_at": datetime.now(timezone.utc),
                    "card_blocked_reason": "User requested temporary block"
                }
            }
...
def unblock_card(account_id):
    ...
    try:
        ...
        if account.get("card_status") == "active":
            return True, "Card is already active." # Not an error, just info
        
        result = accounts_col.update_one(
            {"_id": ObjectId(account_id), "card_status": "blocked_temporary"},
            {
                "$set": {
                    "card_status": "active",
                    "status": "Active", # Reset main status
                    "card_unblocked_at": datetime.now(timezone.utc)
                },
                "$unset": {
                    "card_blocked_at": "", 
        ...
        if result.modified_count > 0:
            ...
            return False, "Failed to unblock card (it might not have been temporarily blocked)"
...
def get_conversation_summaries(user_role, limit=None):
    ...
    try:
        if user_role == "admin":
            ...
            return [] # Only admins and agents can see summaries
        ...
        pipeline = [
            ...
            {"$group": {
                "_id": "$user_id",
                "message_count": {"$sum": 1},
                "last_message_doc": {"$first": "$$ROOT"}, # Get the whole last doc
                "first_timestamp": {"$last": "$timestamp"}
            }},
            {"$sort": {"last_message_doc.timestamp": -1}} # Sort summaries by the last message time
        ]
        if limit:
            pipeline.append({"$limit": limit})
        ...
        for summary in summaries:
            user = get_user_by_id(summary["_id"]) # _id is user_id
            if user:
                last_msg_doc = summary.get("last_message_doc", {})
                ...
                enriched_summaries.append({
                    ...
                    "user_email": user.get("email", "unknown@example.com"),
                    "message_count": summary.get("message_count", 0),
                    "last_message": (last_msg_content[:100] + "...") if len(last_msg_content) > 100 else last_msg_content,
                    "last_timestamp": last_msg_doc.get("timestamp").isoformat() if isinstance(last_msg_doc.get("timestamp"), datetime) else str(last_msg_doc.get("timestamp", "")),
                    "first_timestamp": summary["first_timestamp"].isoformat() if isinstance(summary.get("first_timestamp"), datetime) else str(summary.get("first_timestamp", ""))
                })
...
def get_user_conversation_history(user_id):
    """Get formatted conversation history for a user (for agent view)"""
    if not mongo_connected or messages_collection is None:
        return []
    try:
        messages = list(messages_collection.find({"user_id": user_id}).sort("timestamp", 1))
        ...
        for msg in messages:
            ...
            conversation.append({
                "role": msg.get("role", "user"), # user, bot, or agent
    ...
    try:
        ...
        if user_role == "customer_support_agent":
            ...
            search_filter["user_id"] = {"$in": user_ids}
             ...
             # Only admins and agents can search
            return []
...
def save_feedback(user_id, rating, comment, session_id, issue_resolved):
    ...
    try:
        ...
        # Mark session as feedback_submitted
