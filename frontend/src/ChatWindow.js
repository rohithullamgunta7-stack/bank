...
function ChatWindow({ token, backendUrl }) {
  const [messages, setMessages] = useState([]);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  const [escalationId, setEscalationId] = useState(null);
  ...
  // Derive WebSocket URL from backendUrl
  const WS_BASE_URL = backendUrl.replace('https://', 'wss://').replace('http://', 'ws://');

  console.log("🔧 ChatWindow Configuration:");
  ...
  const checkForExistingEscalation = async () => {
    try {
      console.log("🔍 Checking for existing escalations...");
      const response = await fetch(`${backendUrl}/escalation/escalations/my`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const loadEscalationHistory = async (escId) => {
    try {
      const response = await fetch(`${backendUrl}/escalation/messages/${escId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const handleAccountSelection = async (accountIndex) => {
    ...
    try {
      console.log("📤 Account selection to:", `${backendUrl}/chat`);
    ...
    try {
      if (escalationId) {
        ...
        if (wsRef.current?.readyState === WebSocket.OPEN) {
          ...
          const response = await fetch(`${backendUrl}/escalation/${escalationId}/message?message=${encodeURIComponent(userMessage)}`, {
            method: "POST",
            headers: { Authorization: `Bearer ${token}` }
          });
  ...
  const handleEscalate = async () => {
    try {
      const response = await fetch(`${backendUrl}/escalation/escalate`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
